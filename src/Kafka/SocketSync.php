        ...
        while ($written < $buflen) {
            ...
            if ($writable > 0) {
                ...
                if ($wrote === -1 || $wrote === false) {
                    throw new Socket('Could not write ' . strlen($buf) . ' bytes to stream, completed writing only ' . $written . ' bytes');
                } elseif ($wrote === 0) {
                    // Increment the number of times we have failed
                    $failedWriteAttempts++;
                    if ($failedWriteAttempts > $this->maxWriteAttempts) {
                        throw new Socket('After ' . $failedWriteAttempts . ' attempts could not write ' . strlen($buf) . ' bytes to stream, completed writing only ' . $written . ' bytes');
                    }
                } else {
            ...
            throw new Socket('Could not write ' . strlen($buf) . ' bytes to stream');
        }
        return $written;
    }
